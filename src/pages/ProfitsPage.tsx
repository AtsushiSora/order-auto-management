  ...
  return (
    <>
      ...
      <section className="mini-summary-grid profit-summary">
        ...
        <div className="mini-summary-card teal"><small>予想利益 合計</small><strong>{formatCurrency(expectedTotal)}</strong></div>
      </section>
              ...
              {rows.map(({ vehicle, profit }) => (
                <tr key={vehicle.id}>
                  ...
                  <td className={`number-cell profit-value ${profit.expectedProfit < 0 ? "negative" : ""}`}>
                    <strong>{formatCurrency(profit.expectedProfit)}</strong>
                  </td>
                </tr>
              ))}
            </tbody>
